...
//            }
    ...
    if (HugoUIExecuteRate(&Rate100Hz))
    {
        if (!GyroInit_flag1 && !GyroInit_flag2)
        {
            ...
            printf("pitch = %.1f ", angle_new.x);
            printf("roll = %.1f ", angle_new.y);
            printf("yaw = %.1f\r\n", angle_new.z);
            if (angle_new.x > 10 || angle_new.x < -10)
            {
    ...
    Oled_u8g2_ShowFloat(50, 13, angle_new.x, 2, 2);
    Oled_u8g2_ShowFloat(50, 26, angle_new.y, 2, 2);
    Oled_u8g2_ShowFloat(0, 26, angle_new.z, 2, 2);
